...
class RCControl(object):
    ...
    """
        Drive map : The neural network returns : 
    ...
    def steer(self, prediction):
        if prediction == 2:
            self.serial_port.write("FORWARD")
            print("Forward")
        elif prediction == 0:
            self.serial_port.write("F_LEFT")
            print("Left")
        elif prediction == 1:
            self.serial_port.write("F_RIGHT")
            print("Right")
        else:
            self.stop()
...
class ObjectDetection(object):
    ...
    def detect(self, cascade_classifier, gray_image, image):
        ...
        for (x_pos, y_pos, width, height) in cascade_obj:
            ...
            # stop sign (square sign)
            if width / height == 1:
                cv2.putText(image, 'STOP', (x_pos, y_pos - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

            # traffic lights (rectangular shape)
            else:
                ...
                if maxVal - minVal > threshold:
                    ...
                    # Green light
                        ...
                        cv2.putText(image, 'Yellow', (x_pos + 5, y_pos - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                                    (0, 255, 255), 2)
...
class SensorDataHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        try:
            ...
            print("Connection closed on thread 2 (Sensor data)")
        ...
        try:
            while True:
                ...
                if first != -1 and last != -1:
                    ...
                    # stop conditions (ultrasonic distance less than 3cm)
                    if sensor_data is not None and sensor_data < 30:
                        print("Stop, obstacle in front")
                        self.rc_car.stop()

                    # if there's a stop sign, stop between 0-2.5cm from the sign
                        ...
                        print("Stopped since = %.2fs" % self.stop_time)

                        # 5 seconds later, continue driving
                        if self.stop_time > 5:
                            print("Stopped for 5 seconds")
                            stop_flag = False
                            stop_sign_active = False

                    # if there's a traffic light ahead, wait only if its within 3 cm
            ...
            print("Connection closed on thread 1 (Steering thread)")
...
class ThreadServer(object):
    ...
    video_thread = threading.Thread(target=server_thread, args=(HOST, 8000))
    video_thread.start()
